...
class SpecialItemTest extends Specification {
    def "returns true if item contains special item tag"() {
        given:
            def input = new SpecialItemImpl([new Tag("${SPECIAL_ITEM_TAG_PREFIX}-tag-1"), new Tag( "${SPECIAL_ITEM_TAG_PREFIX}-tag-2")])
        when:
    ...
    def "returns false if item does not contain special item tag"(tags) {
        given:
            def input = new SpecialItemImpl(tags.stream().map({e -> new Tag(e)}).collect(toList()))
        when:
            def output = input.isSpecial()
        then:
            output == false
